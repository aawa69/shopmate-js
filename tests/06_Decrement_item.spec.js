...
describe("--- TEST: DECREMENT SHOPPING LIST ITEM ---", () => {
  it("should check Eggs (DOM) has count of 3 before increment", () => {
    gc.setDisplay();
    const { body } = document;
    expect(body.innerHTML).not.toEqual("");

    const numBefore = document.getElementById("EGGSNo").textContent.replace(/\s+/g, "");
    expect(numBefore).toEqual("3");
  });
